...
### 3d. nerdctl (containerd) is an explicit experimental backend

`--sandbox nerdctl` runs the worker on **containerd** via the `nerdctl` CLI (as found under
Lima or Rancher Desktop). nerdctl is Docker-compatible, so nfind builds the **same** run
command it uses for Docker — generated by the same shared function, including
`--network none` for full network isolation — so the two cannot drift on the
security-critical flags.

It is exercised end-to-end on Linux CI against a real, rootful containerd — a full image
build plus a hardened container run — in addition to nfind's mocked command tests. It stays
marked experimental for two reasons. First, that validation covers only rootful containerd
on a single CI host. Second, rootless nerdctl (RootlessKit + containerd) remaps the host
user to root inside a user namespace just like rootless Podman, which can leave the
read-only mount unreadable by the non-root worker — but unlike Podman, nerdctl has no
`--userns=keep-id` mapping to remap it back, so nfind cannot transparently fix rootless
readability here. Prefer rootful containerd (or Docker/Podman). nfind prints a warning each
time `--sandbox nerdctl` is used.
...
- **Container-runtime vulnerabilities** in Docker, Apple Containers, Podman, or nerdctl. nfind
