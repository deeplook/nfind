...
It is marked experimental for two reasons. First, it has not yet been exercised against a
real containerd runtime, only against nfind's mocked command tests. Second, rootless nerdctl
(RootlessKit + containerd) remaps the host user to root inside a user namespace just like
rootless Podman, which can leave the read-only mount unreadable by the non-root worker — but
unlike Podman, nerdctl has no `--userns=keep-id` mapping to remap it back, so nfind cannot
transparently fix rootless readability here. Prefer rootful containerd, or Docker/Podman,
until this backend is validated. nfind prints a warning each time `--sandbox nerdctl` is
used.
