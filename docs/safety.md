...
The reason it is marked experimental is different from Apple's: the flag set is intended
to be Docker-equivalent, but the backend has not yet been exercised against a real Podman
runtime — only against nfind's mocked command tests. Until that validation happens, nfind
prints a warning each time `--sandbox podman` is used, and Docker remains the most
thoroughly tested backend. Note also that Podman is typically rootless and daemonless; a
rootless container's isolation properties differ from a rootful Docker daemon's, which is
another reason to validate it for your threat model before relying on it.
