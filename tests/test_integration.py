...
from nfind import constants, enumeration, execution, sandbox
...
def base_image() -> str:
    ...
    try:
        execution.build_worker_image()  # default Python base image
...
def test_run_filter_end_to_end(base_image, tmp_path):
    ...
    container_paths, _ = enumeration.enumerate_paths(tmp_path)

    code = "def filter_paths(paths):\n    return [p for p in paths if p.endswith('.txt')]"
    results = execution.run_filter(code, tmp_path, container_paths, image=base_image)
...
def test_run_filter_network_is_disabled(base_image, tmp_path):
    ...
    with pytest.raises(RuntimeError):
        execution.run_filter(code, tmp_path, container_paths, image=base_image)
...
def test_run_filter_times_out(base_image, tmp_path):
    ...
    with pytest.raises(TimeoutError):
        execution.run_filter(code, tmp_path, container_paths, image=base_image, timeout=2)
