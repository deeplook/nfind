...
- **nerdctl (containerd) sandbox backend (experimental).** `--sandbox nerdctl` runs saved
  ...
  the backends cannot drift on the security-critical flags. It is marked experimental
  because it has not yet been validated against a real containerd runtime, and on rootless
