...
- **nerdctl (containerd) sandbox backend (experimental).** `--sandbox nerdctl` runs saved
  and generated filters on containerd via the `nerdctl` CLI (e.g. under Lima or Rancher
  Desktop). nerdctl is Docker-compatible, so nfind applies the *same* hardened run command
  as Docker — generated by the shared Docker-family function, including `--network none`, so
  the backends cannot drift on the security-critical flags. It is exercised end-to-end on
  Linux CI against a real, rootful containerd (image build plus hardened run). It stays
  experimental because that covers only rootful containerd on one CI host, and on rootless
  nerdctl the read-only mount may be unreadable by the non-root worker (nerdctl has no
  `--userns=keep-id` remap like Podman's); nfind prints a warning before running.
